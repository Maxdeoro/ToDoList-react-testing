...
describe('<TodoItem/>', () => {
    ...
    it('Renders <TodoItem/> correctly for completed item', async () => {
        useParams.mockReturnValue({id: 1});
        render (<TodoItem/>);
        await waitForElementToBeRemoved(() => {
            screen.queryByText(/Fetching todo item 1/i);
        });
        expect(axios.get).toHaveBeenCalledTimes(1);
        expect(screen.getByText(/todo item 1/i)).toBeInTheDocument();
        expect(screen.getByText(/Added by: 1/i)).toBeInTheDocument();
        expect(screen.getByText(/This item has been conpleted/i))
                            .toBeInTheDocument();
    });
